...
def get_disk_usage():
    disk_usage = psutil.disk_usage('C:\\')  # Change path to the drive where Windows is installed
...
def update_display():
    ...
    draw_loading_bar(cpu_canvas, 10, 10, cpu_percent, "green", "#1f2131")

    if cpu_percent > 40:
        draw_loading_bar(cpu_canvas, 10, 10, cpu_percent, "orange", "#1f2131")

    if cpu_percent > 70:
        draw_loading_bar(cpu_canvas, 10, 10, cpu_percent, "red", "#1f2131")

    # Change RAM Bar Color Based on Percent

    draw_loading_bar(ram_canvas, 10, 10, ram_percent, "green", "#1f2131")

    if ram_percent > 40:
        draw_loading_bar(ram_canvas, 10, 10, ram_percent, "orange", "#1f2131")

    if ram_percent > 70:
        draw_loading_bar(ram_canvas, 10, 10, ram_percent, "red", "#1f2131")

    # Change temp Bar Color Based on Temp

    draw_loading_bar(temp_canvas, 10, 10, cpu_temp, "green", "#1f2131")

    if cpu_temp > 62:
        draw_loading_bar(temp_canvas, 10, 10, cpu_temp, "orange", "#1f2131")

    if cpu_temp > 76:
        draw_loading_bar(temp_canvas, 10, 10, cpu_temp, "red", "#1f2131")
...
background = Canvas(root, width=200, height=600, background='#18191A', highlightthickness=0)
background.pack()
...
cpu_usage_label = tk.Label(root, text="", font=("Arial", 10), fg="white", bg="#18191A")
cpu_usage_label.place(x=78, y=53)

ram_label = tk.Label(root, text="", font=("Arial", 10), fg="white", bg="#18191A")
ram_label.place(x=78, y=191)

cpu_label = tk.Label(root, text="", font=("Arial", 10), fg="white", bg="#18191A")
cpu_label.place(x=78, y=340)

# Disk usage label

disk_label = tk.Label(root, text="", font=("Arial", 10), fg="white", bg="#18191A")
disk_label.place(x=78, y=470)
